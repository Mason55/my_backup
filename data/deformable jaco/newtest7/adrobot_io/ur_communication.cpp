...
void UrCommunication::run() {
	...
	while (keepalive_) {
		...
		if (keepalive_) {
		  ...
		  printf("Secondary port: No connection. Is controller crashed? Will try to reconnect in 10 seconds...\n"); // Changed by Jiang
                        sec_sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
			if (sec_sockfd_ < 0) {
			...
			while (keepalive_ && !connected_) {
				...
				if (flag_ < 0) {
				  ...
				  printf("Error re-connecting to port 30002. Is controller started? Will try to reconnect in 10 seconds...\n"); // Changed by Jiang
				} else {
					connected_ = true;
