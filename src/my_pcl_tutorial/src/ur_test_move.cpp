...
class CloudFit
{
  ...
  Matrix4f base2cam_mat = Matrix4f::Identity ();
  ...
  Matrix4f init_cam2base_mat = Matrix4f::Identity ();
...
public:
  ...
  void cloudCb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
  {
    ...
    PointCloudT::Ptr cloud_base (new PointCloudT);
    ...
    params.boundary_weight = 0.01;
     ...
     try
      {   
        listener.lookupTransform("/base"/*target_frame*/, "/camera"/*source_frame*/,\
        ...
        base2cam_mat = cam2base_mat.inverse();
      }
    ...
    pcl::transformPointCloud (*cloud_cam/*in*/, *cloud_base/*out*/, cam2base_mat);
    Matrix4f base2cam_mat = cam2base_mat.inverse();
    CloudFit::pointCloud2Vector3d (cloud_base, data.interior);
    nurbs = pcl::on_nurbs::FittingSurface::initNurbsPCABoundingBox (order, &data);//在这里完成了nurbs的初始化
    ...
    tra_point[0].x=0.762; tra_point[0].y=-0.056; tra_point[0].z=0.01;
    sor_point = fit.m_nurbs.PointAt(0,0);
    double z_distance = abs(sor_point.z-tra_point[0].z);
    cout<<"sor:"<<sor_point.z<<endl;
    cout<<"tra:"<<tra_point[0].z<<endl;
    if(z_distance<0.4)
    {cout<<"z_distance: "<<z_distance<<endl;
      tool2base_quan[2] += z_distance;
      std::vector<float> array2(tool2base_quan,tool2base_quan+7);
    ...
    for(i=0;i<=NUB_U;i++)//添加等于号是为了包含边界
    {
      for(j=0;j<=NUB_V;j++)
      {
        ...
        curr_paper_point = base2cam_mat*init_paper_point;//转换到相机坐标系下
    ...
    // parper2base_mat=cam2base_mat*base2cam_mat*parper2cam_mat;
