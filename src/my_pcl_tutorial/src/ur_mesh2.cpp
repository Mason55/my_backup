...
class CloudFit
{
  ros::NodeHandle nh_;
  // ros::NodeHandle nh2_;
  ros::Subscriber sub1_;//初始化
  ros::Subscriber sub2_;//曲面拟合
...
public:
  CloudFit()
  {
    sub1_ = nh_.subscribe ("/voxel/grid", 1, &CloudFit::pcaInit, this);
    sub2_ = nh_.subscribe ("/voxel/grid", 1, &CloudFit::currFit, this);
    sub3_ = nh_.subscribe ("/voxel/grid", 1, &CloudFit::cloudCb, this);
...
//   void transPointCloud()
  ...
  void currFit(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
  {
    PointCloudT::Ptr cloud (new PointCloudT);
    pcl::fromROSMsg (*cloud_msg, *cloud);
    int cloud_size=cloud->size();
  ...
  void cloudCb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
  {
    ...
    // // ROS_INFO ("  %lu cloud points in init_tmp_cloud set\n", init_cam_cloud->size ());
    ...
    int cam_cloud_size = init_cam_cloud->size();
    init_tool_cloud->resize(cam_cloud_size);
    PointCloudT::iterator index = init_tool_cloud->begin();
    PointT change_point;
    for (int i= 0;i<cam_cloud_size;++i)
    {
        x1[0] = init_cam_cloud->points[i].x;
        x1[1] = init_cam_cloud->points[i].y;
        x1[2] = init_cam_cloud->points[i].z;
        x1[3] = 1.0;
        // cout<<tool_point[0]<<endl;
        // cout<<tool_point[1]<<endl;
        // cout<<tool_point[2]<<endl;
        x2 = cam2tool_mat*x1;//转换到相机坐标系下
        change_point.x = x2[0];
        change_point.y = x2[1];
        change_point.z = x2[2];
        init_tool_cloud->insert(index, change_point);
        ++index;
    }

    // //在这里删一点或者是添加一点
    // PointCloudT::iterator index = init_tool_cloud->begin()+5;//第五个点的坐标
    // PointT change_point = init_tool_cloud->points[5];
    
    // if(ch%2==0)
    // {
    //     init_tool_cloud->erase(index);
    //     ++ch;
    //     ROS_INFO (" iffffffif %lu cloud points in init_tool_cloud set\n", init_tool_cloud->size ());
    // }
    // else 
    // {
    //     init_tool_cloud->insert(index, change_point);
    //     ++ch;
    //     ROS_INFO (" else else else %lu cloud points in init_tool_cloud set\n", init_tool_cloud->size ());
    // }
