...
class CloudFit
{
  ...
  ON_3dPoint point_1;//从1 和 2 中选一个点作为目标点POINT_3
  ON_3dPoint point_2;
  ON_3dPoint point_3;
  ON_3dPoint point_4;
  Vector4f in_point;
...
//   std::vector<float> array2(tool_in_base_quan,tool_in_base_quan+7);
...
// /*\测试目标点   
    // */
    // point_2 = fit.m_nurbs.PointAt(0.5,0.5);//
    // point_3 = point_2;

    tra_point[0].x=0.685; tra_point[0].y=0.022; tra_point[0].z=0.056;
    point_1 = fit.m_nurbs.PointAt(0,0.5);//要根据初始化进行调整
    point_2 = fit.m_nurbs.PointAt(1,0.5);//
    cout<<"point_1 z:"<<point_1.z<<endl;
    cout<<"point_2 z:"<<point_2.z<<endl;
    cout<<"point_3 z:"<<point_3.z<<endl;
    cout<<"~~~~~~~~"<<endl;
    if(point_1.z<point_2.z)point_3=point_1;//初始化有两种情况
    else point_3=point_2;
     
    in_point[0] = point_3.x;
    in_point[1] = point_3.y;
    in_point[2] = point_3.z;
    in_point[3] = 1;
    out_point = cam2base_mat*init2curr_mat*in_point;
    point_4.x = out_point[0];
    point_4.y = out_point[1];
    point_4.z = out_point[2];
    cout<<"point_4 z:"<<point_4.z<<endl;
    cout<<"point_4 x:"<<point_4.x<<endl;
    cout<<"point_4 y:"<<point_4.y<<endl;

    double z_distance = tra_point[0].z - point_4.z;//就是Z轴一共要移动的距离
    cout<<"z_distance: "<<z_distance<<endl;
    // cout<<"sor:"<<point_1.z<<endl;
    // cout<<"tra:"<<tra_point[0].z<<endl;
    if(z_distance>-0.05)
    {
        tool_in_base_quan[2] += static_cast<double>(z_distance/20);
        std::vector<float> array2(tool_in_base_quan,tool_in_base_quan+7);
        cout<<"array2"<<array2[0]<<endl;
        cout<<"..........."<<endl;
