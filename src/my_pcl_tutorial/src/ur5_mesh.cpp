...
pcl::visualization::CloudViewer visualizer("test");
...
public:
  CloudFit()
  {
    // sub3_ = nh_.subscribe ("/voxel/grid", 1, &CloudFit::camToTool, this);
    // sub2_ = nh_.subscribe ("/voxel/grid", 1, &CloudFit::pcaInit, this);//2 删掉
  ...
  void camToTool(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)//1
  {
      ROS_INFO("camToTool works/n");
      try
      {   
  ...
  {
    ROS_INFO("pointCloud2Vector3d works/n");
  ...
  {
      ROS_INFO("print4x4Matrix works/n");
  ...
  void cloudCb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
  {
    ...
    printf ("  %lu cloud points in cloud set\n", cloud->size ());

    if(cloud->size()>=cloud_size &&init_flag==1) //读写锁，要确保东西的确是拷贝完毕！
    {       
    ...
    printf ("  %lu cloud points in init_cam_cloud set\n", init_cam_cloud->size ());
    pthread_mutex_lock(&lock);
    pcl::transformPointCloud (*init_cam_cloud/*in*/, *init_tool_cloud/*out*/, cam2tool_mat);//要确保转换的确是做完了！
    pthread_mutex_unlock(&lock);
    visualizer.showCloud(init_tool_cloud,"1");
    visualizer.showCloud(init_cam_cloud,"2");
    printf ("  %lu cloud points in init_tool_cloud set\n", init_tool_cloud->size ());

    pcl::on_nurbs::NurbsDataSurface data;//curr_data
    pcl::on_nurbs::NurbsDataSurface pca_data;//pca的数据
