...
class CloudFit
{
  ...
  Matrix4f tool_init2curr_mat = Matrix4f::Identity ();
  Matrix4f tool_curr2init_mat = Matrix4f::Identity ();
...
//   ON_3dPoint pub_point[4];//要发布的路径点（暂时没包含）
  int init_flag = 1;//pcaInit是否进行的状态标志
  
public:
  CloudFit()
  {
    /*订阅/voxel/grid，初始化nurbs和获得初始TCP姿态*/
  ...
  void pcaInit(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
  {
      pcl::fromROSMsg (*cloud_msg, *init_cam_cloud);
      int cloud_size=init_cam_cloud->size();
      if(init_cam_cloud->size()>=cloud_size &&init_flag==1) //确保有足够的数据点，只进入该函数一次
      {
        *init_cloud = *init_cam_cloud;//缓存到init_cloud中
        ...
        init_flag=0;//初始化完成，设置为0，不在进入该函数
      }
      //下面注释部分是为了测试点云是否在ROS里面可视，确保初始化成功，不影响后面结果
  ...
  void cloudCb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)/*该函数被调用后不断的循环*/
  {
    ...
    tool_curr2init_mat=init_tool2base_mat*(curr_tool2base_mat.inverse());//tool坐标系当前位置到初始位置的变换
    tool_init2curr_mat=tool_curr2init_mat.inverse();//tool坐标系初始位置到当前位置的变换

    pcl::fromROSMsg (*cloud_msg, *cloud_cam);
    //这里要添加一步转化为tool0坐标系下的点云
    pcl::transformPointCloud (*cloud_cam/*in*/, *cloud_init/*out*/, tool_curr2init_mat);//将相机坐标系下当前的点云转化到相机坐标系下的初始点云位置
    ...
    for(i=0;i<=NUB_U;i++)//添加等于号是为了包含边界
    {
      for(j=0;j<=NUB_V;j++)
      {
        ...
        curr_paper_point = tool_curr2init_mat*init_paper_point;//转换到相机坐标系下，为了描点
